...
## 2025.3.2
**蓝桥杯**：
写模拟题三uart，知道了如何输出ascll码，使上位机在文本模式下能显示。但是，如何发出多个tx——data，应该如何去控制，没有攻克

## 2025.3.3
**蓝桥杯**：
如何uart如何收发多个数据，上位机成功在文本模式下显示字符，明天完成模拟题三，有时间加入FIFO控制
...
**蓝桥杯**：
写模拟题三uart，打印出了count:,满足题目要求。比赛时可能要记忆字母的ascll码

## 2025.3.5
**蓝桥杯**：
完成模拟题三uart，改变波特率发现有bug，原来是波特率计数器位宽不对
...
## 2025.3.6
**智能车**：
复习c++基础：命名空间，缺省函数与函数重载，引用；this指针，面向对象基础知识：封装，继承，多态；配opencv环境
